<x-app-layout>
  ...
  <x-slot:content>
    <div class="my-10 container mx-auto w-[500px]">
      <form action="{{ route('product.store') }}" enctype="multipart/form-data" method="POST" class="flex items-center flex-col p-8 gap-8 border-2 -tracking-tighter border-[#1C315E] rounded-lg">
        @csrf
        <h1 class="text-4xl font-bold text-[#1C315E]">{{ __('Tambah Produk') }}</h1>
        <input type="text" autocomplete="off" name="nama" placeholder="Nama Produk" class="border-2 w-full rounded-lg border-[#1C315E] p-5 placeholder:text-[#1C315E]">
        @error('nama')
        ...
        @error('harga')
          <p class="text-red-500 italic">{{ $message }}</p>
        @enderror
        <textarea placeholder="Deskripsi" autocomplete="off" name="deskripsi" class="border-2 w-full rounded-lg border-[#1C315E] p-5 placeholder:text-[#1C315E]"></textarea>
        @error('deskripsi')
          <p class="text-red-500 italic">{{ $message }}</p>
        @enderror
