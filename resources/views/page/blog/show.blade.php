    ...
    <div class="container mx-auto my-[4.5rem]">
      <div class="flex flex-col items-center gap-5">
        ...
        <div class="text-[#1C315E] px-5 gap-3 flex flex-col w-1/2">
          ...
          <h3 class="">{{ $data->slug }}</h3>
          <p class=''>{!! $data->isi !!}</p>
          <form action="{{ route('blog.destroy', $data->id) }}" method="POST" class="flex items-center justify-center gap-5">
            @csrf
            @method("delete")
            <a href="{{ route('blog.edit', $data->id) }}" class="p-3 border-2 border-[#1C315E] hover:bg-[#1C315E] hover:text-[#C0EEE4] text-[#1C315E] rounded-lg transition duration-200">Edit</a>
